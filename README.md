...
### Marcel法とは
Tom Tangoが考案した成績予測手法。過去3年の成績を **5/4/3** で加重平均し、リーグ平均への回帰と年齢調整を行います。
...
同じ選手セット（打者PA≥100 / 投手IP≥30）で統一して比較。

| モデル | OPS MAE (n=172) | ERA MAE (n=145) |
|---|---|---|
| **Marcel法** | **.063** | **0.78** |
| XGBoost | .063 | 0.93 |
| LightGBM | .066 | 0.92 |
...
> 投手ERA予測ではMarcel法がMLを上回り、打者OPS予測では両者がほぼ同等でした。
...
- [x] 予測の不確実性を「幅」で表示（Monte Carloシミュレーションで多様化効果を反映）
...
しかし実際の初年度成績には大きなばらつきがあるため、**Monte Carloシミュレーション（5,000回）** で予測幅を算出しています。

| 選手タイプ | 不確実性の扱い |
|---|---|
| **前リーグ成績あり**の外国人 | ベイズ推定（Shrinkageモデル）の事後分布からサンプリング → wRAA/RA変動 → RS/RAに反映 |
| **前リーグ成績なし**の外国人 | リーグ平均 + 観測ノイズの事後分布からサンプリング |
| **新人（ルーキー）** | 勝数空間で直接 Normal(0, σ=1.17) の摂動を加算 |

```
各サンプルで:
  sim_RS = 中心推定RS + Σ(各選手のwRAA摂動) × スケール係数
  sim_RA = 中心推定RA + Σ(各選手のRA摂動) × スケール係数
  sim_wins = ピタゴラス勝率(sim_RS, sim_RA) × 143 + ルーキー摂動

予測幅 = 勝数分布の80%区間（10〜90パーセンタイル）
```

**多様化効果**: 複数選手の不確実性を単純に足し合わせると過大評価になりますが、MCシミュレーションでは独立な不確実性が部分的に相殺されるため、より現実的な予測幅が得られます（例: 3名独立時の予測幅は線形加算の約58%）。
