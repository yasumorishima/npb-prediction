...
**実成果物を測った被覆率**（`predict_*` を回し直して実際の 2026 シーズンと突き合わせ。名目 0.80 / 0.95・区間を持つ行のみ）:

| | 旧（平の σ） | 新 |
|---|---|---|
| 打者 2026（131 行） | 0.954 / 0.992 | **0.840 / 0.947** |
| 投手 2026（121 行） | 0.950 / 0.992 | **0.843 / 0.967** |

2025 は予測 CSV が残っていないので実成果物では測れない。Marcel 残差から区間を組み直した代理では 打者 0.988/1.000 → 0.876/0.976、投手 0.986/1.000 → 0.847/0.993。**上の表とは別の測り方なので並べて比べない。**
...
⚠️ **σ の fit は 打者 PA≥100 / 投手 IP≥30 でしか行っていない。** 実際には予測 311 人のうち 124 人（40%）が PA<100、302 人のうち 143 人（47%）が IP<30 で、そのまま当てると検証していない領域へ外挿になる（PA=1 では旧来の平の σ より広くなる）。`sigma_model.pt_floor` で**台の下端の値で頭打ち**にしてある。
