...
ローカル / Docker で起動可能（RPi5 での常時稼働は現在停止中）。
...
正本は repo の `data/`（git 管理）。クラウドミラーとして HF Dataset [npb-stats](https://huggingface.co/datasets/yasumorishima/npb-stats) に `data/` ツリーを自動同期（BQ 時代の `npb/` スナップショットも保持）。
...
### 信頼区間の幅（2026-09-21 改訂）

区間の σ は 2018-2025 の全出場機会水準で fit した残差（`sigma_residual`）を全選手に同じ値で当てていた。出場機会で絞った選手にはこれが広すぎる。実測で確かめた結果:

| 確認 | 結果 |
|---|---|
| 実測残差 sd ÷ 当てていた σ（2018-2025 の 8 シーズン） | 打者 **0.531〜0.698** / 投手 **0.561〜0.701**（**8 年すべて 1 未満**）。σ は `posteriors.json` の `sigma_residual`（打者は **wOBA 尺度 0.06215 ×2.33 = 0.14481 OPS**・投手は ERA 尺度 1.66495） |
| 出場機会が増えると残差 sd は下がるか | 8 年すべてで Yes（例 2018 投手 IP 30-44 で 1.42 → 95-168 で 0.62） |

そこで σ を出場機会の関数にした（`posteriors.json` の `sigma_model`。式 `sigma_base * exp(gamma * z_log_pt)` は外国人経路が元から使っていた形）。**2018-2024 だけで fit し、2025 と 2026 は fit に一切使っていない**。入力は 3 月時点で分かる Marcel 予測 PA / IP。

**実成果物を測った被覆率**（`predict_*` を回し直して 2026 シーズンと突き合わせ。名目 0.80 / 0.95）。**採点したのは「区間を持つ行」かつ「実績 PA≥100 / IP≥30」の行だけ**＝この母集団の外にも区間は出しているが、そこは測っていない:

⚠️ **2026 は暫定**＝2026-09-21 時点で 143 試合中 129〜136 消化（95%）でシーズンが終わっていない。最終値ではない。
⚠️ **2026 の行はこのリポジトリの中身だけでは再計算できない**＝採点に使った 2026 の実績はシーズン途中なので commit していない（`data/raw/` は 2025 年までで、`tools/fit_sigma_model.py` は `--current-*` を渡さないと `HELD-OUT 2026: skipped` と出る）。2025 の行は `python tools/fit_sigma_model.py` で再現できる。2026 はシーズン終了後に実績を取り込んで測り直す。

| | 旧（平の σ） | 新 |
|---|---|---|
| 打者 2026（暫定・131 行） | 0.954 / 0.992 | **0.840 / 0.947** |
| 投手 2026（暫定・121 行） | 0.950 / 0.992 | **0.851 / 0.975** |

2025 は予測 CSV が残っていないので実成果物では測れない。Marcel 残差から区間を組み直した代理では 打者 0.988/1.000 → **0.888/0.976**、投手 0.993/1.000 → **0.861/0.986**。**上の表とは別の測り方なので並べて比べない。**

同時に区間の作り方の不具合を 3 件直した。

1. **区間は `stan` を中心に作られていたのに、公表する点推定は BMA 合成の `bayes_*`** だった。凍結データで `|bayes − stan|` は ERA 最大 0.760（σ の 0.65 倍相当）で、ずれは小さくない。
2. **正規分布の分位点を固定 seed の 5,000 draws から取っていた**ため、標本分位点のずれ（seed 42 で 95% −0.0191σ / 80% −0.0331σ）が**全選手に同じ向きで**乗っていた。旧出力の平の σ に当てると **95% −0.0028 OPS / 80% −0.0048 OPS**。閉じた形の分位点に替えて解消（外国人経路も同時に揃えた）。
3. ERA 下限の 0 クリップを再センタリングの後へ移した。

いずれも点推定の値は変えていない（`marcel_*` / `stan_*` / `bayes_*` の全列が 1 つも変わらないことを確認済み）。

⚠️ **σ の fit は 打者 PA≥100 / 投手 IP≥30 でしか行っていない。** 実際には予測 311 人のうち 124 人（40%）が PA<100、302 人のうち **152 人（50%）が IP<30**（runtime が食わせる生の予測 IP で数えた値）で、そのまま当てると検証していない領域へ外挿になる。`sigma_model.pt_floor` で**台の下端の値で頭打ち**にしてあり、**係数もこの頭打ちを当てた上で fit している**（足切りは実績で行うので、実績は閾値を超えたのに予測が下端未満という訓練行が打者 194・投手 240 ある。当てる変換で fit しないとそこが食い違う）。

🔴 **予測 IP に投球回の表記換算を当ててはいけない。** 実績 IP は NPB 表記（"10.2" = 10 と 2/3 回）だが、**Marcel が書く予測 IP は平均の小数**で、実データでは小数第 1 位が 3 以上の行が 61% ある。換算すると 25.9 → 28.0 と壊れる。

⚠️ **区間の幅は `stan` の残差 sd 由来、中心は BMA 合成**という割り切りが残っている。実測の被覆率で追認しているが、幅の根拠と中心の定義は別物。

⚠️ **`data/projections/foreign_*_2026.csv` の区間は旧方式（標本分位点）のまま。** 凍結したので再生成していない。実測（**n=8**）で**上下の半幅の差**が平均 +0.0025 OPS（＝**中心のずれ自体は +0.00125**・最大 0.0045）、95%/80% の幅の比が **1.5221**（閉じた形なら 1.5294）。

再現用: `tools/measure_residual_sd.py`（8 シーズンの残差 sd）と `tools/fit_sigma_model.py`（係数の fit と held-out 検証）。検算は `tests/test_sigma_model.py`。

⚠️ `data/projections/*_2026.csv` は **2026-03-23 に凍結した当時のまま**（区間も旧 σ のまま）。実際に行われたシーズンに対する採点の記録なので再生成していない。次回 `annual_update.yml` を回したときに新しい σ で生成される。
...
| **自動実行** | GitHub Actions（毎年3月1日 + 手動） | データ取得→学習→保存→W&B記録→Gitコミット→HF同期が全自動 |
...
Step 3: fetch_rosters.py        → 支配下登録名鑑取得（FA・移籍確定後）
Step 4: pythagorean.py          → 順位表・ピタゴラス勝率
Step 5: sabermetrics.py         → wOBA/wRC+/wRAA算出
Step 6: marcel_projection.py    → Marcel法予測
Step 7: ml_projection.py        → ML予測 + モデル保存 + メトリクスJSON出力
Step 8: bayes_projection.py     → ベイズ予測（日本人BMA + 外国人Stan v2 + CI）
Step 9: team_simulation.py      → モンテカルロ10,000回チームシミュレーション
git commit & push               → data/ を自動コミット
HF 同期                          → data/ を HF Dataset (npb-stats) へ upload
```

スケジュール: 毎年3月1日 9:00 UTC（FA・移籍確定後、開幕前）+ `workflow_dispatch`（手動実行）

**`hf_only` モード**: `workflow_dispatch` で `hf_only=true` を指定すると、データ再生成を一切走らせず HF 同期のみを実行します（手動 curate 済みの `data/` を上書きしない安全モード）。季中の全量再実行はロースターフィルタ等の手動 curate を上書きしうるため非推奨。
...
- [x] HF Dataset ミラー同期（npb-stats、annual_update.yml に HF upload step + `hf_only` モード）
...
> BigQuery `npb` データセットと BQML モデル（`bqml_batter_ops` / `bqml_pitcher_era` 等）は Grafana 公開廃止に伴い削除。予測ロジックは Python (`scripts/`) に一本化済み、BQ 時代のデータスナップショットは HF Dataset [npb-stats](https://huggingface.co/datasets/yasumorishima/npb-stats) の `npb/` ディレクトリに保持。Streamlit / FastAPI は CSV (`data/projections/`) を直接読むため BQ 非依存で継続稼働。
