...
| 確認 | 結果 |
|---|---|
| 実測残差 sd ÷ 当てていた σ（2018-2025 の 8 シーズン） | 打者 **0.531〜0.698** / 投手 **0.561〜0.701**（**8 年すべて 1 未満**）。σ は `posteriors.json` の `sigma_residual`（0.14481 OPS / 1.66495 ERA） |
...
⚠️ **`data/projections/foreign_*_2026.csv` の区間は旧方式（標本分位点）のまま。** 凍結したので再生成していない。実測で**上下の半幅の差**が平均 +0.0025 OPS（＝**中心のずれ自体は +0.00125**・最大 0.0045）、95%/80% の幅の比が **1.5221**（閉じた形なら 1.5294）。
